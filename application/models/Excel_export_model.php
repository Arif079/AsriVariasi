...
{   
    ...
    public function get_neraca($coa_value, $start = 0, $end = 0) {
        ...
        $this->db->select("
            ...
            COALESCE(SUM(
                CASE 
                    WHEN j1.kdtransaksi LIKE 'sld%' THEN j1.debet
                    ELSE 0
                END
            ), 0) AS saldo_awal,
            COALESCE(SUM(
        ...
        if ($start && $end) {
            ...
            $this->db->where("MONTH(j1.date) IS NULL OR MONTH(j1.date) = ", date("m"));
            $this->db->where("YEAR(j1.date) IS NULL OR YEAR(j1.date) = ", date("Y"));
        }
        ...
        // If no transactions were found, return the COA with default values
        if (empty($result)) {
            $default_result = (object) [
                'coa' => $coa_value,
                'nama' => $this->get_coa_name($coa_value), // A function to fetch COA name
                'saldo_awal' => 0,
                'total_debet' => 0,
                'total_kredit' => 0
            ];
    ...
    // Helper function to get COA name by COA code
    private function get_coa_name($coa_value) {
        $this->db->select('nama');
        $this->db->from('coa');
        $this->db->where('coa', $coa_value);
        $query = $this->db->get();
        $result = $query->row();
        return $result ? $result->nama : 'Unknown COA';
    }
}