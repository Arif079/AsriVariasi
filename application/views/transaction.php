<div class="breadcrumbs">
                    ...
                    <div class="card" id="customerContainer"  style="display:Block;">
                        ...
                        <div class="card-body">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="npwp" id="npwp"style="height:40px" class="form-control form-control-sm" maxlength="16" oninput="this.value = this.value.replace(/[^0-9]/g, '').slice(0, 16);>
                            </div>
                            <div class="form-group">
                                <label>No Telp</label>
                        ...
                        <div class="card-body">
                            <nav>
                                <div class="nav nav-tabs" id="nav-tab" role="tablist">
                                    ...
                                    <a class="nav-item nav-link" id="nav-kacafilm-tab" data-toggle="tab" href="#nav-kacafilm" role="tab" aria-controls="nav-kacafilm" aria-selected="false">Kacafilm</a>
                                    <a class="nav-item nav-link" id="nav-jasa-tab" data-toggle="tab" href="#nav-jasa" role="tab" aria-controls="nav-jasa" aria-selected="false">Jasa</a>
                                </div>
                            </nav>
        ...
        <script>
            ...
            //Change 1
            $('#nav-kacafilm-tab').on("click",function(){
                jQuery("#dataTable").DataTable().ajax.url("<?=base_url("transaction/json_kacafilm");?>").load();
            });
            $('#nav-jasa-tab').on("click",function(){
                jQuery("#dataTable").DataTable().ajax.url("<?=base_url("transaction/json_jasa");?>").load();
            });
            ...
            function refreshServiceCart(data1,data2,data3) {
                ...
                data2.forEach(function(item,index){
                    ...
                    if (item.type === 'kacafilm') {
                    // Custom HTML for special type spare part
                    html2 += '<tr><td>'+item.kditem+'</td><td>'+item.name+'</td><td style="display:none" >Rp '+item.price.toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,')+'</td><td style="display:none" >Rp '+item.priceqq.toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,')+'</td><td class="text-center"><input type="number" style="width:52px" value="'+item.qty+'" class="change-qty" data-hpp="'+item.hpp+'" data-jual="'+item.jual+'" data-sedia="'+item.sedia+'" data-id="'+item.id+'" data-stock="'+item.stock+'"/></td><td><input type="number" style="width:70px" value="'+item.harga+'" id="input" class="change-price" data-id="'+item.id+'"/></td><td><button type="button" class="btn btn-sm btn-danger" onclick="deleteSparepartCart('+item.id+')"><i class="fa fa-times"></i></button></td></tr>';
                    countTotal = (countTotal + (item.harga * item.qty));
                    } else {
                    html2 += '<tr><td>'+item.kditem+'</td><td>'+item.name+'</td><td style="display:none" >Rp '+item.price.toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,')+'</td><td style="display:none" >Rp '+item.priceqq.toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,')+'</td><td class="text-center"><input type="number" style="width:52px" value="'+item.qty+'" class="change-qty" data-hpp="'+item.hpp+'" data-jual="'+item.jual+'" data-sedia="'+item.sedia+'" data-id="'+item.id+'" data-stock="'+item.stock+'"/></td><td><span style="width:70px">Rp ' + (namaQQInput ? item.priceqq : item.price).toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,') + '</span></td><td><button type="button" class="btn btn-sm btn-danger" onclick="deleteSparepartCart('+item.id+')"><i class="fa fa-times"></i></button></td></tr>';
                    if (namaQQInput) {
                        countTotal = (countTotal + (item.priceqq * item.qty));
                    } else {
                        countTotal = (countTotal + (item.price * item.qty));
            ...
            $("body").on('change','.change-price',function(){
                var before = SparepartCart;
                var price = jQuery(this).val();
                var id = jQuery(this).attr("data-id");
                
                if (price > 0 && price !== null) {
                    before[id]["harga"] = price;
                    before[id]["price"] = price;
                    before[id]["priceqq"] = price;
                } else {
                    Swal.fire(
                        'Gagal',
                        'harga tidak boleh kosong atau 0',
                        'error'
                    )
                }
...
// Debounced click event handler
            $(".btn-save-confirm").on("click", debounce(function() {
                if (type == "sparepart") {
                ...
                if (type == "service") {
                    ...
                    form["npwp"] =jQuery("input[name=npwp]").val();
                    ...
                    if (!form["customer"] || !form["sales"] || !form["npwp"] || !form["plat"] || !form["tipe"] || !form["merk"] || !form["noka"] || !form["nosin"] || !form["tahun"] || !form["warna"]) {
                        Swal.fire("Peringatan", "Semua field customer harus diisi!", "warning");
                        ...
                        if (data.status) {
                            ...
                            if (data.type == "sparepart") {
                                ...
                                Swal.fire({
                                    ...
                                    cancelButtonText: 'Print SPK',
                                }).then((result) => {
                                    if (result.value) {
                                        window.location.href = "<?=base_url("wo");?>";
                                    } else {
                                        window.location.href = "<?=base_url("service_sales");?>";
                                    }
                                })
                            }
                        }
