...
class Bkm extends CI_Controller {
    ...
    private function process($action = "add",$id = 0) {
        ...
        // Now you can use $formattedDate in your code
        if (!$coad) {
            ...
            $bkmdata = [
                "id" => NULL,
                "tglbkm" => date("Y-m-d H:i:s"), // This can remain as the current date if needed
                "bkmno" => $bkmno,
                "referensibkm" => $npl,
                "coak" => $coak,
                "coad" => $coad,
            ...
            if ($action == "add") {
                ...
                unset($insertData['id']);
                // unset($insertData['date']); // Uncomment if you want to remove date when editing
